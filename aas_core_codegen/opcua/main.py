"""Generate an OPC UA node set corresponding to the meta-model."""
...
from aas_core_codegen.opcua import naming as opcua_naming
...
def _define_for_enumeration(enumeration: intermediate.Enumeration) -> List[ET.Element]:
    ...
    element = ET.Element(
        "xs:simpleType", {"name": opcua_naming.type_name(enumeration.name)}
    )
    ...
    if isinstance(type_anno.items, intermediate.OurTypeAnnotation):
        ...
        if isinstance(our_type, intermediate.Enumeration):
            ...
            if len(our_type.concrete_descendants) > 0:
                choice_group_name = opcua_naming.choice_group_name(our_type.name)
                xs_group = ET.Element(
                    "xs:group",
                    {
                ...
                xs_element_inner = ET.Element(
                    "xs:element",
                    {
                        ...
                        "type": opcua_naming.type_name(our_type.name),
                        "minOccurs": min_occurs,
                        "maxOccurs": max_occurs,
                    },
                )
    ...
    if isinstance(type_anno, intermediate.PrimitiveTypeAnnotation):
        ...
        if isinstance(our_type, intermediate.Enumeration):
            ...
            if len(our_type.concrete_descendants) > 0:
                ...
                xs_sequence.append(
                    ET.Element(
                        "xs:group", {"ref": opcua_naming.choice_group_name(our_type.name)}
                    )
                )
...
def _generate_xs_group_for_class(
    ...
    for inheritance in cls.inheritances:
        inheritance_xs_group = ET.Element(
            "xs:group", {"ref": opcua_naming.group_name(inheritance.name)}
        )
        xs_sequence.append(inheritance_xs_group)

    xs_sequence.extend(properties)

    xs_group = ET.Element("xs:group", {"name": opcua_naming.group_name(cls.name)})
...
def _define_for_class(
    ...
    xs_group_ref = ET.Element("xs:group", {"ref": opcua_naming.group_name(cls.name)})

    xs_sequence = ET.Element("xs:sequence")
    xs_sequence.append(xs_group_ref)

    complex_type = ET.Element(
        "xs:complexType", {"name": opcua_naming.type_name(cls.name)}
    )
        ...
        xs_choice.append(
            ET.Element(
                "xs:element",
                {
                    ...
                    "type": opcua_naming.type_name(cls.name),
                },
            )
        )
        ...
        xs_choice.append(
            ET.Element(
                "xs:element",
                {
                    ...
                    "type": opcua_naming.type_name(descendant.name),
                },
            )
        )

    xs_group = ET.Element("xs:group", {"name": opcua_naming.choice_group_name(cls.name)})
...
def _generate(
    ...
    root.attrib["xmlns"] = symbol_table.meta_model.xml_namespace
